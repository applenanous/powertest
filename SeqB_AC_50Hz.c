...
void seqB_AC (int test_cycle, status *testStatus);
...
void seqB_AC (int test_cycle, status *testStatus)
{
    for (int n=1; n<=test_cycle; n++)
    {
		...
		for (int i=1; i<=600; i++)
		{
			...
			fflush(stdout);
			wait_ms(i*timeON_step, testStatus);
			//delay(i*timeON_step); 		// Wait i*20 ms
			if (testStatus->state == STOP)
				return;
